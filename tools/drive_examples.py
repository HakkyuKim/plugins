...
def drive_example(plugin_dir):
    ...
    for driver_path in driver_paths:
        ...
        if not test_target_paths:
            driver_relpath = os.path.relpath(driver_path, plugin_dir)
            print(
                f'Found {driver_relpath}, but no integration_test/*_test.dart files.')
            errors.add(f'No test files for {driver_relpath}')
            continue
...
def main(argv):
    ...
    if len(args.plugins) == 0 and args.run_on_changed_plugins:
        ...
        changed_files = subprocess.run(
            f'git diff {base_sha} HEAD', shell=True, cwd=packages_dir, encoding='utf-8', stdout=subprocess.PIPE).stdout.strip().splitlines()
